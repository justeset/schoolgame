...
const CHAT_MESSAGES: Array[Dictionary] = [
	...
	{ "text": "Твоя цель - исправить сбои в системе школы" },
	{ "text": "Используйте WASD или стрелки для перемещения." },
	{ "text": "Подойдите к белой маркерной доске" },
	{ "text": "Нажми E для взаимодействия (и так со всеми предметами)" },
]
...
func _ready() -> void:
	...
	# Настраиваем обработку ввода для пропуска
...
func _play_chat() -> void:
	...
	for i in range(CHAT_MESSAGES.size()):
		...
		# Создаём пузырь с прозрачностью 0
		...
		# Если пропуск - показываем всё сразу
		if _skip_requested:
			print("  -> ПРОПУСК")
		...
		# Ждём окончания анимации или пропуска
		while tw.is_running():
			if _skip_requested:
				tw.kill()
				bubble.modulate.a = 1.0
	...
	_chat_running = false
	solve_button.text = "ПОНЯТНО"
	print("=== _play_chat КОНЕЦ, кнопка: ПОНЯТНО ===")
...
func _on_solve_button_pressed() -> void:
	...
	if _chat_running:
		# Если чат ещё идёт - пропускаем анимацию
		print("Чат ещё идёт, пропускаем анимацию")
		_skip_requested = true
		solve_button.text = "ПОНЯТНО"
	else:
		# Чат закончен - закрываем обучение
