...
func _on_check_button_pressed() -> void:
	...
	if code_text == "":
		_save_code_error("count_orders", "", "Введите код перед проверкой.", 0, "empty_input")
		_show_result_panel(
			false,
			"Пустой ответ",
...
func _on_check_request_completed(result, response_code, headers, body) -> void:
	...
	if json_dict.get("success", false):
		...
		_save_code_error("count_orders", answer_edit.text, explanation, int(json_dict.get("test_number", 0)), error_type)
